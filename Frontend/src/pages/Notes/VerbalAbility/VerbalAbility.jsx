...
import VerbalAbilityData from './VerbalAbilityData';
...
const VerbalAbility = () => {
  ...
  // Icon mapping for each Verbal Ability category
  const getChapterIcon = (chapterId) => {
    const icons = {
      grammar_foundation: BookOpen,
      vocabulary_building: Languages,
      reading_comprehension: FileText,
      para_jumbles: List,
      sentence_correction: Edit,
      fill_blank: Type,
      verbal_analogies: Brain,
      parajumble_para_completion: List,
      word_usage: PenTool,
      verbal_reasoning: MessageSquare,
      business_communication: Mic,
      reading_skills: Search,
      writing_skills: PenTool
    };
    return icons[chapterId] || BookOpen;
  };
  
  // Color mapping for verbal ability categories
  const getChapterColor = (chapterId) => {
    const colors = {
      grammar_foundation: 'blue',
      vocabulary_building: 'green',
      reading_comprehension: 'yellow',
      para_jumbles: 'purple',
      sentence_correction: 'indigo',
      fill_blank: 'pink',
      verbal_analogies: 'red',
      parajumble_para_completion: 'orange',
      word_usage: 'teal',
      verbal_reasoning: 'cyan',
      business_communication: 'amber',
      reading_skills: 'blue',
      writing_skills: 'green'
    };
  ...
  // Filter chapters
  const filteredChapters = VerbalAbilityData.filter((chapter) => {
    const matchesCategory = selectedCategory === 'all' || chapter.id === selectedCategory;
    return matchesCategory;
  });

  // Progress stats
  const totalTopics = VerbalAbilityData.reduce((acc, chapter) => acc + chapter.topics.length, 0);
  const completedCount = completedTopics.size;
  const bookmarkedCount = bookmarkedTopics.size;
  const progressPercentage = Math.round((completedCount / totalTopics) * 100);

  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-emerald-50/30 px-3 py-4 sm:p-4">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-6 sm:mb-8">
          <div className="inline-flex items-center gap-2 sm:gap-3 bg-white/80 backdrop-blur-sm rounded-xl sm:rounded-2xl px-4 sm:px-6 py-2 sm:py-3 shadow-sm border border-gray-200 mb-3 sm:mb-4">
            ...
            <h1 className="text-2xl sm:text-4xl font-bold bg-gradient-to-r from-emerald-600 to-blue-600 bg-clip-text text-transparent">
              Verbal Ability Guide
            </h1>
          </div>
          <p className="text-base sm:text-xl text-gray-600 max-w-3xl mx-auto leading-relaxed px-2">
            Master grammar, vocabulary, reading comprehension, and communication skills for SSC CGL, CAT, and Service Sector Jobs
          </p>
        </div>
          ...
          <div className="flex flex-col gap-4">            
            <div className="flex gap-2 sm:gap-3">
              <select
                ...
                className="flex-1 px-3 sm:px-4 py-3 sm:py-4 border border-gray-200 rounded-lg sm:rounded-xl focus:ring-3 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all text-sm sm:text-base"
              >
                <option value="all">All Categories</option>
                {VerbalAbilityData.map(chapter => (
        ...
        {filteredChapters.length === 0 && (
          <div className="text-center py-12 sm:py-16 bg-white rounded-xl sm:rounded-2xl shadow-lg border border-gray-200">
            ...
            <button
              ...
              className="px-5 sm:px-6 py-2.5 sm:py-3 bg-emerald-600 text-white rounded-lg sm:rounded-xl hover:bg-emerald-700 transition-colors font-medium text-sm sm:text-base"
            >
              Show All Categories
            </button>
          </div>
        ...
        <div className="mt-6 sm:mt-8 text-center text-gray-500 text-xs sm:text-sm">
          <p>
            {filteredChapters.length} of {VerbalAbilityData.length} categories showing • 
